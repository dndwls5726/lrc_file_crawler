...
def lrc_maker():
    ...
    TEXT = TEXT.replace("＃", "\n")
    x = TEXT.count("|")
    with open('%s.lrc' % available_file[i].replace(".mp3", ""), 'w', encoding='UTF8') as file:  # 덮어씌우기
        file.write(TEXT)
    del TEXT
    TEXT = []
    with open('%s.lrc' % available_file[i].replace(".mp3", ""), 'r', encoding='UTF8') as file:  # 한줄씩 읽어오기
        for j in range(0, x):
    ...
    with open('%s.lrc' % available_file[i].replace(".mp3", ""), 'w', encoding='UTF8') as file:  # 초기화
        file.write('')
    for j in range(0, x):
        with open('%s.lrc' % available_file[i].replace(".mp3", ""), 'a', encoding='UTF8') as file:  # 최종
            if j != x:
...
def lrc_delete():
    ...
    fail += 1
    track_clear()
    os.remove('%s.lrc' % available_file[i].replace(".mp3", ""))
    print("%s 은(는) 싱크가사를 지원하지 않습니다." % available_file[i])
    return fail


for file in glob.glob("*.mp3"):  # flac파일 개수 파악
    FILE_LIST.append(file)

if len(FILE_LIST) != 0:  # FLAC 곡이 있는지 확인하기
    for i in range(0, len(FILE_LIST)):
        if 'album' in EasyID3("%s" % FILE_LIST[i]).keys() and 'artist' in EasyID3("%s" % FILE_LIST[i]).keys() and 'title' in EasyID3("%s" % FILE_LIST[i]).keys():  # 아티스트, 앨범, 타이틀 태그 있는지 확인
            FILE_TAG_LIST = EasyID3("%s" % FILE_LIST[i])
    ...
    for i in range(0, len(available_file)):
        ...
        if available_second_artist[i] == '':
            if artist_artistid == album_artistid and artist_artistid in track_artistid:
                ...
                urllib.request.urlretrieve('http://api.bugs.co.kr/3/tracks/%s/lyrics?&api_key=b2de0fbe3380408bace96a5d1a76f800' % track_trackid[n],"%s.lrc" % available_file[i].replace(".mp3", ""))
                with open('%s.lrc' % available_file[i].replace(".mp3", ""), encoding='UTF8') as json_file:
